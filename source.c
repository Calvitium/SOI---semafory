...
void menu()
{
    while(QUIT == 0)
    {
        ...
        puts("Choose one of following options:6"
             "");
        ...
        switch(getchar())
        {
            ...
            case '3':
                while(getchar() != '\n');
                puts("Enter name of a file to download [MAX LENGTH 10]: ");
            ...
            case '4':
                while(getchar() != '\n');
                puts("Enter name of a file to upload [MAX LENGTH 10]: ");
                char TempName1[10];
                scanf("%s",TempName1);
...
void defragment()
{
    ...
    fwrite(descriptors,sizeof(descriptor)*super->file_number,1,Disk);
    fclose(Disk);
    puts("Defragmentation successfull");
}